    ...
    <header className="nav">
      <div className="wrapx nav-in">
        ...
        <nav className="nav-pill" aria-label="주요">
          <Link href="/#specialties" className="nav-link">25개 분과</Link>
      ...
      {open && (
        <div className="nav-menu">
          <Link href="/#specialties" onClick={() => setOpen(false)}>25개 분과</Link>
          <Link href="/about" onClick={() => setOpen(false)}>About us</Link>
          <a href="/app" onClick={(e) => { openStart(e); setOpen(false); }}>시작하기 →</a>
          <div className="nav-menu-theme"><ThemeToggle inpill /></div>
        </div>
      )}
    </header>
  );
