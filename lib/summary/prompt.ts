...
4. 청크에 없는 사실은 지어내지 마세요. 수치·시각으로 구체화(모호 금지: "상승" X → "0.8→2.1 (19:22→20:46)" O). 스냅샷보다 추이를 우선 — 추이가 임상적으로 의미 있는 값(신기능·간효소·염증수치·혈역학·혈색소 등)은 여러 시점을 "값→값→값 (시각→시각)" 형태의 짧은 텍스트 시계열로 남기세요. 단 시점이 하나뿐이거나 변화가 임상적으로 무의미한 값은 억지로 시계열로 만들지 말고 간결하게 두세요(짧고 단순한 차트는 굳이 늘리지 않음).
